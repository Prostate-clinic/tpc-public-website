  ...
  return (
    <div className="flex justify-center gap-2" onPaste={handlePaste}>
      {Array.from({ length: 6 }).map((_, i) => (
        <input key={i} ref={(el) => { refs.current[i] = el; }} type="text" inputMode="numeric" maxLength={1} value={value[i] || ""} onChange={(e) => handleChange(i, e.target.value)} onKeyDown={(e) => handleKeyDown(i, e)} className="h-12 w-11 rounded-md border border-gray-300 bg-white text-center text-lg font-bold tracking-widest focus:border-[#1a1aaa] focus:outline-none focus:ring-2 focus:ring-[#1a1aaa]/20 sm:w-12" />
      ))}
    </div>
  );
}
...
function PatientForgotPasswordPageInner() {
  ...
  return (
    <AuthLayout
      title={step === "email" ? "Reset your password" : step === "otp" ? "Set a new password" : "Password reset"}
      subtitle={
        step === "email"
          ? "We'll send a 6-digit code to your email to verify it's you."
          : step === "otp"
            ? <>Code sent to <span className="font-semibold text-gray-900">{email}</span>. It expires in 10 minutes.</>
        ...
        <form onSubmit={handleReset} className="space-y-5">
          <div>
            ...
            <div className="mt-3 flex items-center justify-between">
              <button type="button" onClick={() => setStep("email")} className="text-xs font-medium text-gray-500 hover:text-[#1a1aaa]">Change email</button>
              <button type="button" onClick={handleResend} disabled={resendIn > 0 || loading} className="text-xs font-semibold text-[#1a1aaa] hover:underline disabled:text-gray-400">{resendIn > 0 ? `Resend in ${resendIn}s` : "Resend code"}</button>
            </div>
          </div>
